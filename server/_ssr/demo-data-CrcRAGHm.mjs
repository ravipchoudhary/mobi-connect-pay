//#region node_modules/.nitro/vite/services/ssr/assets/demo-data-CrcRAGHm.js
var STORAGE_KEYS = {
...
function getDemoSliderImages() {
	const images = readStorage(STORAGE_KEYS.sliderImages, []);
	if (images.length === 0) {
		const defaults = [
			{
				id: crypto.randomUUID(),
...
function addDemoSliderImage(input) {
	const next = [...getDemoSliderImages(), {
		id: crypto.randomUUID(),
		...input
	}];
...
function updateDemoSliderImage(id, input) {
	const next = getDemoSliderImages().map((item) => item.id === id ? {
		...item,
		...input
	} : item);
	writeStorage(STORAGE_KEYS.sliderImages, next);
...
function deleteDemoSliderImage(id) {
	const next = getDemoSliderImages().filter((item) => item.id !== id);
	writeStorage(STORAGE_KEYS.sliderImages, next);
	return next;
}
function toggleDemoSliderImageActive(id) {
	const next = getDemoSliderImages().map((item) => item.id === id ? {
		...item,
		active: !item.active
	} : item);
...
//#endregion
export { markNotificationRead as _, addDemoTicket as a, updateDemoDmt2RequestStatus as b, getDemoActivity as c, getDemoNotifications as d, getDemoPermissions as f, getDemoWalletSummary as g, getDemoTransactions as h, addDemoSliderImage as i, getDemoApiKeys as l, getDemoTickets as m, addDemoDmt2Request as n, addDemoTransaction as o, getDemoSliderImages as p, addDemoNotification as r, deleteDemoSliderImage as s, addDemoApiKey as t, getDemoDmt2Requests as u, toggleDemoApiKey as v, updateDemoSliderImage as x, toggleDemoSliderImageActive as y };
